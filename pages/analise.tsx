...
export default function AnaliseCompleta() {
  ...
  // Diferente de pages/admin/print-profile.tsx (de onde este arquivo foi derivado): aqui o
  // e-mail NUNCA vem da URL — vem exclusivamente da sessão do próprio candidato logado
  // (mesmo padrão de autenticação usado em pages/my-results.tsx e pages/ninebox-public.tsx).
  // Isso garante que um candidato só consiga ver a própria Análise, nunca a de outra pessoa,
  // mesmo que tente editar a URL manualmente.
  ...
  useEffect(() => {
    ...
    const sessionEmail = sessionStorage.getItem('aderenciaEmail');
    if (!role || !sessionEmail) { router.push('/login'); return; }
  ...
  return (
    <>
      ...
      <div className="print-bar no-print">
        ...
        <button onClick={() => router.push('/my-results')} style={{ background: '#475569' }}>← Voltar aos meus resultados</button>
      </div>
