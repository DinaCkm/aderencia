...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const participants: ParticipantProfile[] = readJson('participants', []);
  const performances: PerformanceRecord[] = readJson('performance', []);
  const discReports: DiscReport[] = readJson('discReports', []);
  ...
  const results = (participant.selectedAreas || []).map((area) => {
    ...
    // Extrair pontuações dos calculationSteps para exibição transparente
